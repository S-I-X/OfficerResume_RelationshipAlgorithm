...
def create_relationship(graph=None, node1=None, node2=None, rel_type=None, property=None, value=None):
    """
    函数功能：在图中创建关系，并可设置属性。
    :param graph: 图数据库连接
    :param node1: 节点1
...
def init_countrymen(graph=None, mysql=None):
    """此方法暂时停用
        ...
        for i in range(len(person_group) - 1):  # 给人物两两创建同乡关系
            for j in range(i + 1, len(person_group)):
                if not person_group[i]['person']['id'] == person_group[j]['person']['id'] and not \
                        my_cur.execute(select_sql.format(person_group[i]['person']['id'], person_group[j]['person']['id'])):
...
def init_schoolfellow(graph=None, mysql=None):
    """
    ...
    for node_school in graph.find(label='School'):  # 查找学校节点
        ...
        person_group = graph.data(f"MATCH (person:Person)-[r:study_at]->{node_school} RETURN person, r;") \
                       + graph.data(f"MATCH (person:Person)-[r:study_at]->(academy:Academy)<-[:include_academy]-{node_school} "
        ...
        for i in range(len(person_group) - 1):  # 匹配教育经历时间，并创建同学关系
            ...
            for j in range(i + 1, len(person_group)):
                ...
                if overlap:
                    if not my_cur.execute(select_sql.format(person_group[i]['r']['study_id'], person_group[j]['r']['study_id'])):
                        create_relationship(graph, person_group[i]['person'], person_group[j]['person'],
                                            'schoolfellow_with', 'cost', 5/(4*(1+math.log(overlap[2], math.e))*(6-float(type_int))))  # 创建同学关系
                        # 插入关系型数据库保存结果
                        insert_value = insert_value + \
                                       ...
                                       'NULL,' if len(person_group[i]) == 2 else f"'{person_group[i]['academy']['id']}'," + \
                                       'NULL,' if len(person_group[i]) == 2 else f"'{person_group[i]['academy']['name']}'," + \
                                       f"'{node_school['id']}','{node_school['name']}'," + \
                                       'NULL,' if len(person_group[j]) == 2 else f"'{person_group[j]['academy']['id']}'," + \
                                       'NULL,' if len(person_group[j]) == 2 else f"'{person_group[j]['academy']['name']}'," + \
...
def init_workmate(max_level=1, graph=None, mysql=None):
    """
    ...
    for node_institution in graph.find(label='Institution'):  # 查找所有机构节点
        count += 1
        print(time_now(1), f":************************ {count} Institution:{node_institution['name']} ************************")
        # if count < 23059:
        #     continue
        # 查找工作于当前机构顶层职位的所有人物
        worker_group = graph.data(f"MATCH (person:Person)-[r:work_at]->(position:Position)<-[:include_position]-{node_institution} "
        ...
        for i in range(len(worker_group) - 1):  # 匹配工作经历时间，并创建同事关系
            ...
            for j in range(i + 1, len(worker_group)):
                ...
                if worker_group[j]['r']['end_time'][0] == '0':  # 结束时间年份为‘0’的表示至今，按格式更改为当前时间
                    ...
                    if overlap:
                        if not my_cur.execute(select_sql.format(worker_group[i]['r']['work_id'], worker_group[j]['r']['work_id'])):
                            create_relationship(graph, worker_group[i]['person'], worker_group[j]['person'],
                                                'workmate_with', 'cost', 1/(5*(1+math.log(overlap[2], math.e))))  # 创建同事关系
                            # 插入关系型数据库保存结果
                            insert_value = insert_value + \
                            ...
                            if trigger >= 100:
                                ...
                                try:
                                    ...
                                    print(time_now(1), ':\t\tMySQL Insert Error At Counts:', trigger, 'in', rel_num)
        # 查找当前机构节点下的所有职位
        position_group = graph.data(f"MATCH (position:Position)<-[:include_position*..]-{node_institution} RETURN position;")
        for position_up in position_group:  # 查找每个职位节点的多层下级关系
            ...
            if not graph.data(f"MATCH (institution:Institution)-[:include_position]->{position_up} RETURN institution;")[0]:
                for i in range(len(workers) - 1):  # 匹配当前职位中的工作经历时间，并创建同事关系
                    ...
                    for j in range(i + 1, len(workers)):
                        ...
                        if workers[j]['r']['end_time'][0] == '0':  # 结束时间年份为‘0’的表示至今，按格式更改为当前时间
                            ...
                            if overlap:
                                if not my_cur.execute(select_sql.format(workers[i]['r']['work_id'], workers[j]['r']['work_id'])):
                        ...
                        for j in range(len(worker_group) - 1):
                            ...
                            if worker_group[j]['r']['end_time'][0] == '0':  # 结束时间年份为‘0’的表示至今，按格式更改为当前时间
                                ...
                                overlap = period_cmp(workers[i]['r']['start_time'], workers[i]['r']['end_time'],
                                                     worker_group[j]['r']['start_time'], worker_group[j]['r']['end_time'])
                                if overlap:
                                    if not my_cur.execute(select_sql.format(workers[i]['r']['work_id'], worker_group[j]['r']['work_id'])):
                                        create_relationship(graph, workers[i]['person'], worker_group[j]['person'],
                                                            'workmate_with', 'cost', (type_int+1)/(5*(1+math.log(overlap[2], math.e))))  # 创建同事关系
                                        # 插入关系型数据库保存结果
                                        insert_value = insert_value + \
...
def create_one_countrymen(node=None, graph=None):
    """此方法暂时停用
    ...
    type_int = 1  # 同乡关系类型
    while type_int < 10:
        if graph.data(f"MATCH {location}<-[:include_location*{type_int-1}]-(location:Location) RETURN location;")[0]['location']['name'] == '中国':
            break
        type_int += 1
...
def create_one_schoolfellow(node=None, graph=None):
    """
...
def create_one_workmate(node=None, max_level=1, graph=None):
    """
    函数功能：根据给定的人物节点node，在图中查找并建立他的复杂同事关系。
    同事关系类型[0,10]，n：表示x是y的第n层上级，0：表示x与y是同一级别。双向关系只保存一条记录。
    :param node:给定的人物节点
    :param max_level: 最大允许查找下级的层数缺省值
    :param graph: 图数据库连接配置
    ...
    for work_at in works:
        ...
        for person in person_group:
            ...
            if person['r']['end_time'][0] == '0':  # 结束时间年份为‘0’的表示至今，按格式更改为当前时间
                person['r']['end_time'] = time_now(0)  # 当前日期
            # 匹配时间段重叠情况
            overlap = period_cmp(work_at['r']['start_time'], work_at['r']['end_time'], person['r']['start_time'], person['r']['end_time'])
            if overlap:  # 创建同事关系
                # 保存同事关系信息
                workmate_pair.append([work_at['r']['work_id'], person['r']['work_id'],
                                      ...
                                      institution['id'], institution['name'],
                                      person['position']['id'], person['position']['name'], overlap[0], overlap[1], '0'])
                create_relationship(graph, node, person['person'], 'workmate_with', 'cost', 1/(5*(1+math.log(overlap[2], math.e))))  # 创建同事关系
        type_int = 0  # 控制往下查找的层级数
        while type_int < max_level:  # 查找当前节点的多层下级关系
            type_int += 1
            positions_down = graph.data(f"MATCH (position:Position)<-[:include_position*{type_int}]-{position_up} "
...
def add_init(add_id_group=None, graph=None, mysql=None):
    """
    函数功能：根据新增加的人物ID列表，对这些人的三种关系及其代价值进行增量计算。
    ...
    for node in node_group:  # 对每个增加的人物节点进行关系查找
        ...
        graph.data(f"MATCH {locations[0]['location']}<-[r:is_from]-{node} SET r.cost={float(5/location_level/6)} RETURN r;")
        ...
        for schoolfellow in create_one_schoolfellow(node, graph):
            if not schoolfellow[0] == schoolfellow[1] and \
                ...
                insert_value = insert_value + f" ('{schoolfellow[0]}','{schoolfellow[1]}','{schoolfellow[2]}'," \
                                              ...
                                              f"{schoolfellow[9]}," if schoolfellow[9] == 'NULL' else f"'{schoolfellow[9]}'," + \
                                              f"'{schoolfellow[10]}','{schoolfellow[11]}'," + \
                                              f"{schoolfellow[12]}," if schoolfellow[12] == 'NULL' else f"'{schoolfellow[12]}'," + \
                                              f"{schoolfellow[13]}," if schoolfellow[13] == 'NULL' else f"'{schoolfellow[13]}'," + \
                ...
                if trigger >= 100:
                    ...
                    try:
                        ...
                        mysql.rollback()  # 插入失败，执行回滚操作
                        print(time_now(1), ':\t\tSchoolfellow-MySQL Insert Error At Counts:', trigger, 'in', rel_num_schoolfellow)
        if trigger > 0:
            finish_insert_sql = insert_sql_schoolfellow + insert_value
            finish_insert_sql = finish_insert_sql[:-1] + ';'
            try:
