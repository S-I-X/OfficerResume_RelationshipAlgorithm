...
def period_cmp(start_time1=None, end_time1=None, start_time2=None, end_time2=None):
    ...
    if time_int[0] <= time_int[2] <= time_int[1] or time_int[0] <= time_int[3] <=time_int[1]:
        return [start_time1 if time_int[0] >= time_int[2] else start_time2,
                end_time1 if time_int[3] >= time_int[1] else end_time2, 1 if overlap <=1 else overlap]  # 若存在时间重叠，则返回重叠时间段及重叠年数
    else:
...
def searchAndSave_allShortestPaths(person1_id=None, person2_id=None, rel_type_int=[1, 2, 3, 4, 5, 6, 7], limit=10, n_paths=10):
    """
    ...
    if my_cur.execute(select_sql):
        try:
            my_cur.execute(update_sql.format(paths_nodes_id, paths_relationships, paths_directions, person1_id, person2_id))
            mysql.commit()
        ...
        try:
            my_cur.execute(insert_sql.format(person1_id, person2_id, paths_nodes_id, paths_relationships, paths_directions))
            mysql.commit()
